...
O primeiro módulo do sistema a ser detalhado é o módulo de visualização do sinal. Na figura \ref{tela1} pode ser observada a tela em que o usuário seleciona quais canais (ou derivações) do sinal deseja visualizar. No caso, as derivações do sinal são {modified limb lead II} (MLII) e \textit{lead} V1 .
...
A figura \ref{tela2} mostra a impressão no sinal na tela do sistema. Além dos sinais, a tela também apresenta opções para alterar a frequência e amplitude do sinal e opções para mostrar o sinal em janelas de 5 ou 10 segundos.
...
\label{Mod2} 
Uma vez que a tela do sistema exibe o sinal de Eletroencefalografia (ECG), o usuário pode realizar as marcações de eventos.
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.8\textwidth]{./figs/tela4.png} % <- formatos PNG, JPG e PDF
	\caption[Seleção do editor de marcações]{Seleção do editor de marcações}
	\fonte{Autoria Própria}
	\label{tela2}
\end{figure}
