...
O sistema de configuração mais comum em exames ambulatoriais de ECG (Eletrocardiografia) é o sistema 12 derivações, que consiste nas seguintes derivações \cite{MALMIVUO1995}:
...
Além das ondas, os elementos existentes na forma de onda do ECG (Eletrocardiografia) são \cite{GACEK2011}:
...
A Curva \sigla{ROC}{Receiving Operating Characteristic} \textit{(Receiving Operating Characteristic)} é útil para visualização da performance de classificadores. Seu uso vem crescendo em pesquisas nas áreas de \textit{machine learning} e \textit{data mining} \cite{FAWCETT2006}. 
...
O índice DER \textit{(Detection Error Rate)}, dado na equação ~(\ref{eq:DER}) \cite{Zhang2007}, relaciona o número de erros do sistema, soma dos falsos positivos e falsos negativos, com o número total de eventos do sistema.
