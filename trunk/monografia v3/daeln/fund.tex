...
A configuração mais comum em exames ambulatoriais de Eletrocardiografia (ECG) é o sistema 12 derivações, que consiste nas seguintes derivações \cite{MALMIVUO1995}:
...
Existe um tipo especial de redes neurais artificiais auto organizáveis conhecidas como Mapas Auto Organizáveis ou Mapas de Kohonen. Nos Mapas de Kohonen, os neurônios, ou nós, são dispostos em nós em uma estrutura bidimensional e passam por um processo de aprendizado competitivo utilizando padrões de entrada.  Os neurônios são então ordenados uns em relação aos outros. O Mapa de Kohonen forma, portanto, um mapa topográfico dos padrões de entrada, no qual as coordenadas dos neurônios indicam características dos padrões de entrada \cite{HAYKIN2011}.
...
Nesta seção vamos nos concentrar em formas de avaliar o desempenho de sistemas classificadores por meio das seções: \ref{subsec:TabelaCont} Tabela de Contingência, \ref{subsec:SeEsP} Sensibilidade e Especificidade, \ref{subsec:ROCCurva} Curva ROC e \ref{subsec:outros} Outros Índices Estatísticos.
