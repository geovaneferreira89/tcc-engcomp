...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.9\textwidth]{./figs/uml.png} % <- formatos PNG, JPG e PDF
...
O mapa é inicializado com fragmentos obtidos do sinal. Inicialmente, foram realizados testes percorrendo o sinal todo e captando fragmentos deslocando o sinal ponto por ponto. Contudo, isso gerava fragmentos redundantes e aumentava o tempo de duração da etapa de reconhecimento de padrões. Em vista disso, foi decidido deslocar o sinal de três em três pontos para obtenção de cada fragmento. Outro aspecto relevante é que para particionar o sinal de ECG sem extração de caracteríticas, são usadas 50 amostras. No particionamento do sinal de ECG após a extração de características é necessário utilizar 200 amostras.
...
O tamanho do Mapa é variável, contudo, manteve-se um tamanho padrão de 10 por 10 nós para os testes. Nós contendo fragmentos do sinal com características do complexo QRS tenderam a se localizar nas regiões em que as coordenadas \textit{x} e \textit{y} se aproximam de 0. Para separar a região onde se concentra o complexo QRS do restante do mapa, determinam-se valores de corte do \textit{x} e \textit{y}. Por exemplo, os nós com os valores de \textit{x} e \textit{y} entre 0 e 3 representam o grupo de padrões que se deseja buscar. No Capítulo de Resultados Obtidos \ref{sec:Resultados} são apresentados resultados para diferentes valores de corte de \textit{x} e \textit{y}.

O algoritmo do Mapa de Kohonen segue a lógica detalhada no capítulo de Fundamentação Teórica, e o código utilizado no sistema foi obtido em 
